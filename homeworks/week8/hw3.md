...
而非同步執行則是，這個動作開始了之後，直接開始下一個動作，直到 stack 裡清空後，event loop 會把 task queue 裡的 task 再拿出來執行。
...
這就是非同步執行，如果全部的動作都是同步執行的話，在 Google 伺服器傳送搜尋結果回來之前，你的網頁頁面就會像當機一樣，按什麼都沒有反應。
